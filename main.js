...
async function getForecast(city) {
  let req = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=e43a8295e45e4e3cbb4131300240410&q=${city}&days=3`
  );
  ...
  let cartona = `<div class="col-lg-4 ">
     <div class="inner1 rounded-start-2">
      <div class="inner-head d-flex justify-content-between px-2 py-2">
        <span class="">${currentDate.toLocaleDateString("en-US", {
          weekday: "long",
        })}</span>
        <span class="">${currentDate.getDate()} <span class="">${currentDate.toLocaleDateString(
    "en-US",
    { month: "long" }
  )}</span> </span>
      </div>
  ...
  <div class="col-lg-4">
    <div class="${i == 1 ? "inner2" : "inner1"} ${i == 2 ? "inner3" : ""}">
      <div class="inner-head text-center px-2 py-2">
        <span class="text-center">${new Date(array[i].date).toLocaleDateString(
          "en-US",
          { weekday: "long" }
        )}</span>
      </div>
